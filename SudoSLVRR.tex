...
%
\begin{abstract}
    Because of its interdependent constraints, large-scale sudoku puzzles expose limitations of standard and parallel ant colony optimization (ACO) in terms of balancing fast convergence and adequate solution diversity. This paper proposes SudoSLVRR, a multithreaded multi-colony ACO framework that integrates constraint propagation with entropy-driven dynamic collaborative mechanism and dual-topology inter-thread communication. Within each thread, heterogeneous ACS and MMAS colonies cooperate to balance exploration and exploitation while allowing the threads to perform a ring-based iteration-best exchange promoting a structured convergence and randomized best-so-far sharing preventing premature stagnation. Experimental results on $9 \times 9$, $16 \times 16$, and $25 \times 25$ sudoku instances show higher success rate and lesser runtime compared to CP-ACS and CP-DCM-ACO baselines. These findings highlight the effectiveness of structured multi-level collaboration in resolving exponential time complexity associated with large-scale combinatorial optimization.
...
\input{sections/sudoslvr_algorithm}
